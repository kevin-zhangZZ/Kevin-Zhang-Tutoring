...
const EXAM_AI: SAExaminerStats = {
  marks: [10, 90],
  average: 0.9,
  comment: <>An exact answer was required. 0.3125 was sometimes seen.</>,
}

const EXAM_AII: SAExaminerStats = {
  marks: [15, 85],
  average: 0.9,
  comment: <>An exact answer was required. Some students rounded to 0.813.</>,
}
...
const EXAM_AIII: SAExaminerStats = {
  ...
  comment: (
    <>
      ...
      <Katex tex="\Pr(2\le X\le5)" /> in the numerator.
    </>
  ),
}
...
const EXAM_AIV: SAExaminerStats = {
  ...
  comment: (
    <>
      Some wrote down the variance instead of the standard deviation. Common incorrect
      answers were <Katex tex="\tfrac54" /> and 1.118. An exact answer was required.
    </>
  ),
}
...
const EXAM_BI: SAExaminerStats = {
  ...
  comment: (
    <>
      Many students gave{' '}
      <Katex tex="\tfrac{63a}{8}+\tfrac{27b}{8}+\tfrac{3c}{2}" /> as the answer.
    </>
  ),
}
...
const EXAM_CI: SAExaminerStats = { marks: [32, 68], average: 0.7 }

const EXAM_CII: SAExaminerStats = {
  marks: [34, 66],
  average: 0.7,
  comment: <>Some students did not give their answer as an interval.</>,
}
...
const ROWS_AIII: WorkingRow[] = [
  {
    ...
    reason: <>The intersection of <Katex tex="X\ge2" /> and <Katex tex="X<5" /> is <Katex tex="2\le X\le4" /> — including <Katex tex="X=5" /> in the numerator is the report's named error.</>,
  },
  {
...
const ROWS_AIV: WorkingRow[] = [
  ...
  },
  {
    ...
    reason: <>The <em>variance</em> — quoting <Katex tex="\tfrac54" /> as the answer is the report's named slip.</>,
  },
  {
    working: <Katex display tex="\boxed{\mathrm{sd}(X) = \frac{\sqrt5}{2}}" />,
    reason: <>Exact: <Katex tex="\sqrt{5/4}=\tfrac{\sqrt5}{2}\approx1.118" />, but the decimal was not accepted.</>,
  },
]
...
const ROWS_BI: WorkingRow[] = [
  ...
  },
  {
    ...
    reason: <>No integration is required at all. Evaluating the integral in terms of <Katex tex="a,b,c" /> — the report's named response — answers a question that was not asked, and is the first of the three equations in part b(ii).</>,
  },
]
...
const ROWS_BII: WorkingRow[] = [
  ...
  },
  {
    ...
    reason: <>Exact values. <Katex tex="-\tfrac{167}{60}=-2.7833\ldots" />, so the rounded <Katex tex="-2.783" /> was not accepted.</>,
  },
]
...
const ROWS_CI: WorkingRow[] = [
  ...
  },
  {
    ...
    reason: 'Countably many values, so discrete — the justification is part of the mark.',
  },
]
    ...
    <div className="flex flex-col gap-8">
      <div className="text-[14.5px] leading-relaxed text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-800/60 border border-gray-100 dark:border-gray-800 rounded-2xl px-5 py-4 flex flex-col gap-2">
        ...
        <p>
          ...
          landing on tails. Let <Katex tex="X" /> be the binomial random variable
          representing the number of times that the coin lands on heads. Mika flips the coin
          five times.
        </p>
      </div>

      <PartCard letter="a.i" marks={1} statement={<>Find <Katex tex="\Pr(X=5)" />.</>} examinerReport={EXAM_AI}>
        <WorkingTable rows={ROWS_AI} />
      </PartCard>

      <PartCard letter="a.ii" marks={1} statement={<>Find <Katex tex="\Pr(X\ge2)" />.</>} examinerReport={EXAM_AII}>
      ...
      <PartCard
        ...
        statement={
          <>
            ...
            probability density function <Katex tex="g" />, a transformation of{' '}
            <Katex tex="f" /> given by <Katex tex="g(d)=f(rd+s)" />. Find the values of{' '}
            <Katex tex="r" /> and <Katex tex="s" />.
          </>
        }
      ...
      <div className="text-[13.5px] leading-relaxed text-gray-600 dark:text-gray-400 px-1">
        <p>
          ...
          of <Katex tex="p" /> of landing on heads. Bella flips her coin 25 times in order to
          estimate <Katex tex="p" />. Let <Katex tex="\hat P" /> be the random variable
