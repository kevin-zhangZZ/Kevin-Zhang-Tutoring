...
// plotted roots and circle are this site's own answer, drawn with matplotlib.
...
import argandSrc from './spec-2019exam2-q2-argand.png'

const EXAM_AI: SAExaminerStats = {
  marks: [32, 68],
  average: 0.7,
  comment: <>Most students used the quadratic formula. As a "show that", sufficient working had to be presented.</>,
}
...
const EXAM_BI: SAExaminerStats = {
  marks: [28, 17, 55],
  average: 1.3,
  comment: <>Most students recognised that the two solutions had to be at opposite ends of a diameter for the radius to be a minimum.</>,
}

const EXAM_BII: SAExaminerStats = {
  marks: [41, 59],
  average: 0.6,
  comment: <>Some students did not square the radius correctly when converting to cartesian form.</>,
}
...
const ROWS_AI: WorkingRow[] = [
  ...
  },
  {
    ...
    reason: <>Dividing each term by <Katex tex="4" />. Every line matters here — it is a "show that".</>,
  },
]
  ...
  },
  {
    working: (
      <div className="bg-white border border-gray-200 dark:border-gray-800 rounded-xl p-3 w-fit">
        <img src={argandSrc} alt="Argand diagram with the conjugate pair −1 ± (√6/2)i plotted and the circle of minimum radius through them, centred at −1" className="w-full max-w-[330px]" />
      </div>
    ),
    reason: <>The two points, together with the circle from part (b) drawn through them. Conjugate roots are always mirror images in the real axis.</>,
  },
]
...
const ROWS_BI: WorkingRow[] = [
  ...
  },
  {
    ...
    reason: <>Centre <Katex tex="-m=-1" /> gives <Katex tex="m=1" />.</>,
  },
]
...
const ROWS_BII: WorkingRow[] = [
  ...
  },
  {
    ...
    reason: <>Squaring: <Katex tex="\left(\tfrac{\sqrt6}{2}\right)^2=\tfrac{6}{4}=\tfrac32" />. Forgetting to square the radius is the error the report flags.</>,
  },
]
...
const ROWS_C: WorkingRow[] = [
  ...
  },
  {
    ...
    reason: <>This branch is easy to miss, but the question says <Katex tex="d\in\mathbb{R}" /> and asks only that the solutions satisfy the relation — it never says they must be non-real. Real roots sit on the real axis and can perfectly well fall inside the circle.</>,
  },
  {
    working: (
      <>
...
const ROWS_D: WorkingRow[] = [
  ...
  },
  {
    ...
    reason: <>The radius is the size of the imaginary part. VCAA's published answer writes this as <Katex tex="\tfrac{\sqrt{4ac-b^2}}{2a}" />, which is the same thing when <Katex tex="a>0" />; the report's own comment lists "not accounting for the sign of <Katex tex="a" />" as a common error, so the absolute value is the safe form — a radius can never be negative.</>,
  },
]
  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="text-[14.5px] leading-relaxed text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-800/60 border border-gray-100 dark:border-gray-800 rounded-2xl px-5 py-4">
        ...
        <p>
          Consider the equation <Katex tex="2z^2+4z+5=0" />, where <Katex tex="z\in\mathbb{C}" />.
        </p>
      </div>

      <PartCard letter="a.i" marks={1} statement={<>Show that the solutions of <Katex tex="2z^2+4z+5=0" />, where <Katex tex="z\in\mathbb{C}" />, are <Katex tex="z=-1\pm\dfrac{\sqrt6}{2}i" />.</>} examinerReport={EXAM_AI}>
        <WorkingTable rows={ROWS_AI} />
      </PartCard>

      <PartCard letter="a.ii" marks={1} statement={<>Plot the solutions of <Katex tex="2z^2+4z+5=0" /> on the Argand diagram below.</>} examinerReport={EXAM_AII}>
        <WorkingTable rows={ROWS_AII} />
      </PartCard>

      <div className="text-[14.5px] leading-relaxed text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-800/60 border border-gray-100 dark:border-gray-800 rounded-2xl px-5 py-4">
        <p>
          Let <Katex tex="|z+m|=n" />, where <Katex tex="m,n\in\mathbb{R}" />, represent the
          circle of minimum radius that passes through the solutions of{' '}
          <Katex tex="2z^2+4z+5=0" />.
        </p>
      </div>

      <PartCard letter="b.i" marks={2} statement={<>Find <Katex tex="m" /> and <Katex tex="n" />.</>} examinerReport={EXAM_BI}>
        <Background>
          <p>
      ...
      <PartCard letter="b.ii" marks={1} statement={<>Find the cartesian equation of the circle <Katex tex="|z+m|=n" />.</>} examinerReport={EXAM_BII}>
        <WorkingTable rows={ROWS_BII} />
      </PartCard>

      <PartCard letter="b.iii" marks={1} statement="Sketch the circle on the Argand diagram in part a.ii. Intercepts with the coordinate axes do not need to be calculated or labelled." examinerReport={EXAM_BIII}>
      ...
      <PartCard letter="c" marks={2} statement={<>Find all values of <Katex tex="d" />, where <Katex tex="d\in\mathbb{R}" />, for which the solutions of <Katex tex="2z^2+4z+d=0" /> satisfy the relation <Katex tex="|z+m|\le n" />.</>} examinerReport={EXAM_C}>
        <Background>
          <p>
          ...
          <p>
            ...
            be checked, which is why almost every student who attempted it found only one
            endpoint.
          </p>
        </Background>
      ...
      <PartCard letter="d" marks={2} statement={<>All complex solutions of <Katex tex="az^2+bz+c=0" /> have non-zero real and imaginary parts. Let <Katex tex="|z+p|=q" /> represent the circle of minimum radius in the complex plane that passes through these solutions, where <Katex tex="a,b,c,p,q\in\mathbb{R}" />. Find <Katex tex="p" /> and <Katex tex="q" /> in terms of <Katex tex="a" />, <Katex tex="b" /> and <Katex tex="c" />.</>} examinerReport={EXAM_D}>
