...
// the answer. Answers checked with sympy and against the VCAA examination report. Solution
// is original.
...
const EXAM_A: SAExaminerStats = {
  ...
  comment: (
    <>
      Most students correctly placed and labelled the points. A small number either placed
      them incorrectly, labelled them incorrectly, or neglected to label them.
    </>
  ),
}
...
const EXAM_B: SAExaminerStats = {
  ...
  comment: (
    <>
      Most students realised that the complex conjugate of the given solution was also a
      solution. Some algebraic errors were observed.
    </>
  ),
}
...
const EXAM_C: SAExaminerStats = {
  ...
  comment: (
    <>
      Students who used comparison of coefficients to find the quadratic factor were
      generally more successful than those who used long or synthetic division. Students who
      completed the square rather than using the quadratic formula were also generally more
      successful.
    </>
  ),
}
...
const ROWS_A: WorkingRow[] = [
  ...
  {
    working: <Katex display tex="\boxed{\text{see the plot below}}" />,
    reason: 'Both points labelled — the mark was for labelling as much as for placement.',
  },
]
...
const ROWS_C: WorkingRow[] = [
  {
    ...
    reason: 'Comparing coefficients is quicker and safer here than long division.',
  },
  {
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <PartCard
        ...
        statement={
          <>
            Consider <Katex tex="z_1=1+2i" />. Plot and label <Katex tex="z_1" /> and{' '}
            <Katex tex="\overline{z_1}" /> on the Argand plane.
          </>
        }
      ...
      >
        ...
        <div className="bg-white border border-gray-200 dark:border-gray-800 rounded-xl p-3 w-fit">
          <img
            src={argandSrc}
            alt="An Argand plane with two labelled points at 1 + 2i and 1 − 2i, one above the other on the vertical line through Re(z) = 1"
