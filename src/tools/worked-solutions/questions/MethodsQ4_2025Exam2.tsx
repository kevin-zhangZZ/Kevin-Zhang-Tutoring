...
// the Newton sketch is our own drawing of the answer. Answers checked with sympy/scipy and
...
import newtonSrc from './meth-2025e2-q4eii-sketch.png'
...
const EXAM_A: SAExaminerStats = {
  ...
  comment: (
    <>
      This question was answered well. Some students left their answer unsimplified or gave
      an approximate value when an exact answer was required.
    </>
  ),
}
...
const EXAM_B: SAExaminerStats = {
  ...
  comment: (
    <>
      Some responses had only the first one or two solutions. Others incorrectly gave extra
      solutions or a general solution, not considering the restricted domain.
    </>
  ),
}
...
const EXAM_C: SAExaminerStats = {
  ...
  comment: (
    <>
      Many students were able to find the <Katex tex="k" /> value but not the{' '}
      <Katex tex="a" /> value.
    </>
  ),
}
...
const EXAM_D: SAExaminerStats = {
  ...
  comment: (
    <>
      An equation was required. Some students inefficiently attempted to find the equation by
      hand, rather than using their CAS, and made errors.
    </>
  ),
}
...
const EXAM_EII: SAExaminerStats = {
  ...
  comment: (
    <>
      Many students did not attempt this question. Some drew lines which were not tangential
      to the curve at any point. Students should use a ruler when drawing lines.
    </>
  ),
}
...
const EXAM_FI: SAExaminerStats = {
  ...
  comment: (
    <>
      This was a "show that" question and appropriate working needed to be shown. Many
      students were able to find the gradient but did not express the equation correctly.
    </>
  ),
}
...
const EXAM_FII: SAExaminerStats = {
  ...
  comment: (
    <>
      Exact answers were required. Some students gave their answers in coordinate form
      without stating the minimum and maximum values of the <Katex tex="y" />-intercept.
      Others labelled the values the wrong way round.
    </>
  ),
}
...
const EXAM_FIII: SAExaminerStats = {
  ...
  comment: (
    <>
      <Katex tex="p=\tfrac{3\pi}{2}" /> was often included in the response and was not
      awarded full marks.
    </>
  ),
}
...
const EXAM_GII: SAExaminerStats = {
  ...
  comment: (
    <>
      This question was well done by those who attempted it. Some students gave their answer
      in exact form, not correct to two decimal places as required.
    </>
  ),
}
...
const EXAM_GIII: SAExaminerStats = {
  marks: [67, 19, 14],
  average: 0.5,
  comment: <>Exact answers were required; some students gave only approximate answers.</>,
}
...
const ROWS_C: WorkingRow[] = [
  ...
  },
  {
    ...
    reason: <>This is the half most students missed: <Katex tex="f(x+2\pi)" /> only exists while <Katex tex="x+2\pi" /> stays in the domain.</>,
  },
  {
...
const ROWS_EII: WorkingRow[] = [
  ...
  {
    working: <Katex display tex="\boxed{\text{see the drawing below}}" />,
    reason: <>A ruled line, touching the curve at <Katex tex="x=x_1" /> and cutting the axis at <Katex tex="x_2" />. Lines that were not actually tangential earned nothing.</>,
  },
]
...
const ROWS_FI: WorkingRow[] = [
  ...
  },
  {
    ...
    reason: <>Point–gradient form. Writing <Katex tex="y-f(p)=f'(x)(x-p)" /> instead — with the derivative left as a function of <Katex tex="x" /> — was the listed error.</>,
  },
  {
...
const ROWS_FII: WorkingRow[] = [
  ...
  },
  {
    ...
    reason: <>So <Katex tex="p=\pi" /> is a maximum and <Katex tex="p=2\pi" /> a minimum — getting them the wrong way round was a listed error.</>,
  },
  {
...
const ROWS_FIII: WorkingRow[] = [
  ...
  },
  {
    ...
    reason: <>Two decimal places. Including <Katex tex="\tfrac{3\pi}{2}\approx4.71" /> was the error the examiner singled out.</>,
  },
]
...
const ROWS_GI: WorkingRow[] = [
  ...
  },
  {
    ...
    reason: <>The first condition. Substituting anything other than <Katex tex="x=0" /> here was the listed error.</>,
  },
  {
...
const ROWS_GII: WorkingRow[] = [
  ...
  },
  {
    ...
    reason: <>Two decimal places, as required — an exact form scored nothing here.</>,
  },
]
    ...
    <div className="flex flex-col gap-8">
      <div className="text-[14.5px] leading-relaxed text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-800/60 border border-gray-100 dark:border-gray-800 rounded-2xl px-5 py-4 flex flex-col gap-3">
        ...
        <p>
          ...
          <Katex tex="f:\left[0,\tfrac{5\pi}{2}\right]\to\mathbb{R}" />,{' '}
          <Katex tex="f(x)=\sin(x)+1" />. The graph of <Katex tex="y=f(x)" /> is shown below.
        </p>
      ...
      <PartCard letter="a" marks={1} statement={<>Evaluate <Katex tex="f\!\left(\tfrac{2\pi}{3}\right)" />.</>} examinerReport={EXAM_A}>
      ...
      <PartCard
        ...
        statement={
          <>
            ...
            <Katex tex="f(x+k)=f(x)" /> for all <Katex tex="x\in[0,a]" />. Find the value of{' '}
            <Katex tex="k" /> and the largest possible value of <Katex tex="a" />.
          </>
        }
      ...
      <PartCard
        ...
        statement={
          <>
            ...
            <Katex tex="A" /> where <Katex tex="x=\tfrac{2\pi}{3}" />, as shown below. Find
      ...
      >
        <div className="bg-white border border-gray-200 dark:border-gray-800 rounded-xl p-3 w-fit">
          <img
            src={tangentSrc}
            alt="The same curve with a dashed straight line touching it at the labelled point A where x = 2π/3 and sloping down to the right — from the original 2025 VCAA exam paper"
      ...
      <PartCard
        ...
        statement={
          <>
            Apply two iterations of the Newton method to <Katex tex="f" /> with{' '}
            <Katex tex="x_0=\tfrac{2\pi}{3}" />. Write down <Katex tex="x_2" />, correct to
            one decimal place.
          </>
        }
      ...
      >
        ...
        <div className="bg-white border border-gray-200 dark:border-gray-800 rounded-xl p-3 w-fit">
          <img
            src={newtonSrc}
            alt="The curve with the given tangent at x₀ shown faintly and the second tangent at x₁ drawn in, cutting the x-axis at x₂ ≈ 5.2"
            className="w-full max-w-[560px]"
          />
        </div>
      </PartCard>
      ...
      <PartCard
        ...
        statement={
          <>
            ...
            of <Katex tex="y=f(x)" />. Give your answers correct to two decimal places.
          </>
        }
      ...
      <div className="text-[14.5px] leading-relaxed text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-800/60 border border-gray-100 dark:border-gray-800 rounded-2xl px-5 py-4">
        <p>
          ...
          <Katex tex="g:\left[0,\tfrac{5\pi}{2}\right]\to\mathbb{R}" />,{' '}
          <Katex tex="g(x)=ax^3+bx^2+cx+d" /> be a polynomial function, where{' '}
          <Katex tex="a,b,c,d\in\mathbb{R}" />. Suppose <Katex tex="g(0)=f(0)" /> and{' '}
          <Katex tex="g'(0)=f'(0)" />.
        </p>
      </div>

      <PartCard letter="g.i" marks={2} statement={<>Show that <Katex tex="c=1" /> and <Katex tex="d=1" />.</>} examinerReport={EXAM_GI}>
      ...
      <PartCard
        ...
        statement={
          <>
            ...
            <Katex tex="y=g(x)" />, for <Katex tex="x\in[0,2\pi]" />. Give your answer correct
      ...
      <PartCard
        ...
        statement={
          <>
            Let <Katex tex="a=0" />, <Katex tex="c=1" />, <Katex tex="d=1" />. Find{' '}
            <Katex tex="b" /> and <Katex tex="r" />, such that <Katex tex="g(r)=f(r)" /> and{' '}
            <Katex tex="g'(r)=f'(r)" />, where <Katex tex="b\in\mathbb{R}" /> and{' '}
