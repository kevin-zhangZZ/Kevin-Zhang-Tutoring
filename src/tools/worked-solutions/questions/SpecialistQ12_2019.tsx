...
const EXAMINER: MCQExaminerStats = {
  ...
  comment: (
    <>
      ...
      in a null set, as it is not possible for a resolute of a vector to be of greater magnitude
...
const ROWS: WorkingRow[] = [
  ...
  },
  {
    ...
    reason: <>Note the question only asks which equations you would <em>solve</em>, not for the solution. As the report points out, this system actually has no solution: the resolute <Katex tex="2\underset{\sim}{i}-3\underset{\sim}{j}+\underset{\sim}{k}" /> has magnitude <Katex tex="\sqrt{14}\approx3.7" />, but a projection can never be longer than the original vector <Katex tex="\underset{\sim}{a}" />, whose magnitude is only <Katex tex="\sqrt3\approx1.7" />.</>,
  },
]
    ...
    <MCQShell
      question={
        <p>
          ...
          <Katex tex="m" />, <Katex tex="n" /> and <Katex tex="p" /> are real constants. The
          values of <Katex tex="m" />, <Katex tex="n" /> and <Katex tex="p" /> can be found by
          solving the equations
        </p>
      }
      options={[
        { letter: 'A', content: <Katex tex="\tfrac{m(m+n-p)}{m^2+n^2+p^2}=2,\ \tfrac{n(m+n-p)}{m^2+n^2+p^2}=-3,\ \tfrac{p(m+n-p)}{m^2+n^2+p^2}=1" />, isAnswer: true },
        { letter: 'B', content: <Katex tex="\tfrac{m(m+n-p)}{m^2+n^2+p^2}=1,\ \tfrac{n(m+n-p)}{m^2+n^2+p^2}=1,\ \tfrac{p(m+n-p)}{m^2+n^2+p^2}=-1" /> },
        { letter: 'C', content: <Katex tex="m+n-p=6,\ m+n-p=-9,\ m+n-p=-3" /> },
        { letter: 'D', content: <Katex tex="m+n-p=3m,\ m+n-p=3n,\ m+n-p=-3p" /> },
        { letter: 'E', content: <Katex tex="m+n-p=2\sqrt3,\ m+n-p=-3\sqrt3,\ m+n-p=\sqrt3" /> },
      ]}
      rows={ROWS}
