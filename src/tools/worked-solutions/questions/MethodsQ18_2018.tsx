...
const ROWS: WorkingRow[] = [
  {
    working: <Katex display tex="\text{let } r=\tfrac{p}{q}, \ s=\tfrac{m}{n}, \quad f(x)=x^r, \ g(x)=x^s" />,
  ...
  {
    working: <Katex display tex="f(x)>g(x) \text{ on } (0,1), \quad g(x)>f(x) \text{ on } (1,\infty) \;\iff\; 0<r<s" />,
  ...
  {
    working: <Katex display tex="h(x)=g(x)-f(x)=x^s-x^r \;\implies\; h'(x)=sx^{s-1}-rx^{r-1}=x^{r-1}\bigl(sx^{s-r}-r\bigr)" />,
    reason: <>Factor out the smaller power to compare <Katex tex="f'" /> and <Katex tex="g'" /> in one expression: <Katex tex="h'(x)>0 \iff g'(x)>f'(x)" />.</>,
  },
  {
    working: <Katex display tex="x>1 \;\implies\; x^{s-r}>1 \;\implies\; sx^{s-r}>s>r \;\implies\; sx^{s-r}-r>0" />,
  ...
  {
    working: <Katex display tex="\boxed{h'(x)>0 \text{ for every } x>1 \;\implies\; g'(x)\ne f'(x) \text{ anywhere on } (1,\infty)}" />,
  ...
  {
    working: <Katex display tex="\text{but on } (0,1): \ x^{s-r}<1 \;\implies\; sx^{s-r}-r \text{ can be either sign}" />,
