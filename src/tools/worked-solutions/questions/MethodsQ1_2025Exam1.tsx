...
const EXAM_A: SAExaminerStats = {
  ...
  comment: (
    <>
      This question was well attempted. Many students did not tidy up the negative signs in
      their answer. Some students did not use brackets around terms, which meant that these
      presentations had the potential to be misinterpreted.
    </>
  ),
}
...
const EXAM_B: SAExaminerStats = {
  ...
  comment: (
    <>
      ...
      students did not correctly identify the initial power of the term as{' '}
      <Katex tex="\tfrac12" />. Some students proceeded further to find the equation of the
      tangent line instead of only finding the gradient.
    </>
  ),
}
...
const ROWS_B: WorkingRow[] = [
  {
    ...
    reason: <>Writing the root as a power first. The initial index is <Katex tex="\tfrac12" />, not 1 — misreading it was a listed error.</>,
  },
  {
  ...
  },
  {
    ...
    reason: <><Katex tex="8+1=9" />, a perfect square — the numbers are chosen so no calculator is needed.</>,
  },
  {
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="rounded-2xl border border-gray-200 dark:border-gray-800 p-5 sm:p-6">
        <Background>
          <p>
            Two standard derivatives, one each way. The marks here go to presentation as much
            as to method: brackets around <Katex tex="\bigl(-\sin(x)\bigr)" /> while you write
      ...
      <PartCard
        ...
        statement={
          <>
            Let <Katex tex="y=x^2\cos(x)" />. Find <Katex tex="\dfrac{dy}{dx}" />.
          </>
        }
      ...
      <PartCard
        ...
        statement={
          <>
            Let <Katex tex="f(x)=6\sqrt{x+1}+5" />. Find the gradient of the tangent to{' '}
