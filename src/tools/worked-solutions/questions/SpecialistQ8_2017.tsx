...
// the hardest question on Section A after MCQ 10, and the most popular answer (D, 52%) was
// wrong. Where the gradient of x³ − mx² + 4 is strictly increasing. Question text
...
const ROWS: WorkingRow[] = [
  ...
  },
  {
    ...
    reason: <>Dividing by <Katex tex="6" />: <Katex tex="\tfrac{2m}{6}=\tfrac{m}{3}" />. Option D's <Katex tex="\tfrac{2m}{3}" /> keeps the <Katex tex="2" /> in the numerator — a single arithmetic slip that half the state made.</>,
  },
  {
    working: <Katex display tex="\boxed{x\ge\frac{m}{3}}" />,
    reason: <>Option B. Why the endpoint is included: <Katex tex="f''\left(\tfrac{m}{3}\right)=0" />, but <Katex tex="f''>0" /> on either side of it, so <Katex tex="f'" /> is still <em>strictly</em> increasing across the whole of <Katex tex="\left[\tfrac{m}{3},\infty\right)" /> — a single point of zero second derivative does not create a flat stretch.</>,
  },
  {
    ...
    <MCQShell
      ...
      background={
        <p>
          ...
          being concave up. More than half of all students answered the first question here
