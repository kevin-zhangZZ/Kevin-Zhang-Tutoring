...
const EXAM: SAExaminerStats = {
  ...
  comment: (
    <>
      The most direct approach was to form a quadratic equation set equal to zero, use the
      null factor law to factorise and then solve. Most students set up the quadratic equation
      correctly; however, some did not proceed to factorise and solve. Some students arrived
      at <Katex tex="e^x=6" /> or <Katex tex="e^x=-2" /> and then gave both{' '}
      <Katex tex="x=\log_e6" /> and <Katex tex="x=\log_e(-2)" /> as their solutions, without
      discarding the second. Students need to keep in mind that <Katex tex="\log_e(a)" /> only
      exists for <Katex tex="a>0" />.
    </>
  ),
}
...
const ROWS: WorkingRow[] = [
  ...
  },
  {
    ...
    reason: 'This rejection must be written down — a bare pair of answers loses the mark, and log_e(−2) does not exist.',
  },
  {
  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="text-[14.5px] leading-relaxed text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-800/60 border border-gray-100 dark:border-gray-800 rounded-2xl px-5 py-4">
        ...
        <p>
          Solve <Katex tex="e^{2x}-12=4e^x" /> for <Katex tex="x\in\mathbb{R}" />.
        </p>
      </div>
