...
const EXAMINER: MCQExaminerStats = {
  ...
  comment: (
    <>
      Solving <Katex tex="f'(x)=0" /> assuming no cancellation requires a negative
      discriminant. If <Katex tex="h=-1" /> or <Katex tex="h=4" /> then the fraction cancels
      to a linear function, which also has no turning points.
    </>
  ),
}
...
const ROWS: WorkingRow[] = [
  ...
  },
  {
    ...
    reason: <>The endpoints are special: the numerator cancels a factor, leaving a hyperbola with no turning points either. So both belong in the answer.</>,
  },
  {
    working: <Katex display tex="\boxed{-1 \le h \le 4}" />,
    reason: <>Option <b>C</b>. It is the closed brackets that make this question interesting — spotting the cancellation is what separates it from option <b>B</b>.</>,
  },
]
    ...
    <MCQShell
      question={
        <p>
          ...
          <Katex tex="h\in\mathbb{R}" />, will have no turning points when
        </p>
      }
      options={[
